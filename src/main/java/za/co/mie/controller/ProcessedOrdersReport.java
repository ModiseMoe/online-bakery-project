...
@WebServlet(name = "ProcessedOrdersReport", urlPatterns = {"/ProcessedOrdersReport"})
public class ProcessedOrdersReport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Order> processedOrderList = bsci.getAllProceesedOrders();

        // Set the content type to PDF
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", "attachment; filename=Processed Orders.pdf");

        // Create the PDF document using iText
        Document document = new Document(PageSize.A4);
        PdfWriter.getInstance(document, response.getOutputStream());
        document.open();
        document.addTitle("Processed Orders Report");
        ...
        Paragraph heading = new Paragraph("Processed Orders Report", headingFont);
        ...
        for (Order order : processedOrderList) {
...
// Helper method to get the current date and time
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("processedOrderList", processedOrderList);

        // Forward the request to the JSP file
        request.getRequestDispatcher("processedOrdersReport.jsp").forward(request, response);
    }
